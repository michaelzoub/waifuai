  ...
  return (
    <main className={`${dark ? `flex flex-col w-full min-h-screen md:overflow-hidden md:h-screen bg-zinc-900 text-white ${loading ? "overflow-hidden" : "overflow-visible"}` : `flex flex-col w-full min-h-screen overflow-visible md:overflow-hidden md:h-screen bg-zinc-100 text-black ${loading ? "overflow-hidden" : "overflow-visible"}`}`}>
      ...
      <div className={`w-full md:w-[20%] h-[350px] md:mt-[0px] mt-[-30px] md:h-[100%] pb-4 my-auto z-50 border-l-[0px] md:border-l-[1px] ${dark ? "border-zinc-600 bg-zinc-900" : "border-zinc-300 bg-zinc-100 md:bg-zinc-50"}`}>
        ...
        <div className={`${dark ? "bg-zinc-900 w-full h-[200px] md:h-[7%] flex flex-row md:flex-col gap-0 md:gap-1" : "bg-zinc-50 w-full h-[200px] md:h-[7%] flex flex-row md:flex-col gap-0 md:gap-1"}`}>
          <input className={`${dark ? "mx-1 md:mx-auto border-[1px] w-full md:w-[90%] h-fit border-black rounded-lg px-2 py-1 bg-zinc-700" : "mx-1 md:mx-auto border-[1px] w-full md:w-[90%] h-fit border-black rounded-lg px-2 py-1 bg-zinc-50"}`} placeholder="Talk to waifu!" value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onSubmit={sendButton}></input>
